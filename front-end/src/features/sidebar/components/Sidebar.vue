<template>
  ...
  <aside class="sidebar" :class="{ 'sidebar--open': sidebarOpen }">
    ...
    <nav class="sidebar-nav">
      <RouterLink class="nav-item" :class="{ 'nav-item--active': route.name === 'dashboard' }" to="/dashboard" @click="toggleSidebar(false)">
      ...
      <RouterLink v-if="!isAgent" class="nav-item" :class="{
        'nav-item--active': route.name === 'helpdesk' && !route.query.id,
      }" to="/helpdesk" @click="handleHelpdeskClick; toggleSidebar(false)">
      ...
      <template v-if="helpdeskStore.history.length">
        ...
        <RouterLink v-for="c in helpdeskStore.history" :key="c.id" class="nav-item nav-item--history"
          ...
          <span class="nav-history-label">
            ...
            <span class="nav-history-preview">{{
      ...
      <RouterLink v-if="isAgent" class="nav-item" :class="{ 'nav-item--active': route.name === 'helpdesk-agent' }"
        to="/helpdesk/agent" @click="handleAgentClick; toggleSidebar(false)">
        <span class="nav-item-icon-wrap">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
