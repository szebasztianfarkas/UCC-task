<template>
  ...
  <aside class="sidebar" :class="{ 'sidebar--open': sidebarOpen }">
    ...
    <nav class="sidebar-nav">
      <RouterLink class="nav-item" :class="{ 'nav-item--active': route.name === 'dashboard' }" to="/dashboard"
      ...
      <RouterLink v-if="!isAgent" class="nav-item"
        :class="{ 'nav-item--active': route.name === 'helpdesk' && !route.query.id }" to="/helpdesk"
      ...
      <template v-if="helpdeskStore.history.length">
        ...
        <RouterLink v-for="c in helpdeskStore.history" :key="c.id" class="nav-item nav-item--history"
          :class="{ 'nav-item--active': route.query.id === String(c.id) }"
          :to="{ name: 'helpdesk', query: { id: c.id } }" @click="toggleSidebar(false)">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
      ...
      <RouterLink v-if="isAgent" class="nav-item" :class="{ 'nav-item--active': route.name === 'helpdesk-agent' }"
        to="/helpdesk/agent" @click="toggleSidebar(false)">
        <span class="nav-item-icon-wrap">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
  ...
  <div class="sidebar-backdrop" :class="{ 'sidebar-backdrop--visible': sidebarOpen }" @click="toggleSidebar(false)" />
</template>
...
onMounted(async () => {
  ...
  await helpdeskStore.fetchUnreadCounts()
