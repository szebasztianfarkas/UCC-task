<template>
  <article class="event-card" :class="{ 'event-card--past': isPast }">
    ...
    <div class="card-footer">
      <div class="attendees">
        <div v-for="(a, i) in visibleAttendees" :key="a.id" class="attendee-bubble"
          :style="{ zIndex: visibleAttendees.length - i }" :title="a.username">
      ...
      <template v-if="!event.is_owner">
        ...
        <button v-else class="btn-leave" @click="$emit('leave', event)">
          Leave
        </button>
      </template>
...
const month = computed(() =>
  date.value.toLocaleString("default", { month: "short" }).toUpperCase(),
);
const time = computed(() =>
  date.value.toLocaleString("default", {
    hour: "numeric",
    minute: "2-digit",
    hour12: false,
    weekday: "short",
...
const visibleAttendees = computed(() =>
  props.event.attendees.slice(0, MAX_BUBBLES),
);
const overflowCount = computed(() =>
  Math.max(0, props.event.attendee_count - MAX_BUBBLES),
);
</script>
