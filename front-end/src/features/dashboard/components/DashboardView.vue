<template>
  <div class="page-root">
    ...
    <main class="page-main">
      <header class="page-topbar">
        <div class="page-topbar-left">
          ...
          <span class="event-count">{{ store.events.length }} event{{
            store.events.length !== 1 ? "s" : ""
          }}</span>
        </div>
        <button class="btn-primary" @click="openCreate">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16px" height="16px">
      ...
      <div class="filters-bar">
        <div class="search-wrap search-bar">
          <input v-model="search" class="search-bar__input" type="text" placeholder="Search events…"
            @input="onSearchInput" />
          <button v-if="search" class="search-bar__clear" @click="
            search = '';
          applyFilters();
          ">
      ...
      <div v-else class="events-grid">
        <EventCard v-for="event in store.events" :key="event.id" :event="event" @edit="openEdit" @delete="confirmDelete"
          @join="handleJoin" @leave="handleLeave" />
      </div>
    </main>

    <AppModal :open="modal.open" :title="modal.mode === 'create' ? 'New event' : 'Edit event'" @close="closeModal">
      <form @submit.prevent="submitModal" novalidate>
        <div class="field" :class="{ 'field--error': modalErrors.title }">
          ...
          <span v-if="modalErrors.title" class="field-error">{{
            modalErrors.title
            }}</span>
        </div>
        <div class="field" :class="{ 'field--error': modalErrors.occurrence }">
          ...
          <span v-if="modalErrors.occurrence" class="field-error">{{
            modalErrors.occurrence
            }}</span>
        </div>
        <div class="field">
        ...
        <div v-if="modalErrors.general" class="alert alert--error">
          {{ modalErrors.general }}
        </div>
    ...
    <AppModal :open="!!deleteTarget" title="Delete event?" size="sm" @close="deleteTarget = null">
      <p class="delete-msg">
      ...
      <div class="modal-actions">
        ...
        <button class="btn-danger" :disabled="deleteLoading" @click="executeDelete">
          <span v-if="!deleteLoading">Delete</span><span v-else class="spinner" />
        </button>
      </div>
    </AppModal>
  </div>
...
const search = ref("");
const filters = reactive<EventFilters>({ upcoming: false, attending: false, createdBy: false });
...
async function submitModal() {
  if (!modalForm.title.trim()) {
    modalErrors.title = "Title is required.";
    return;
  }
  if (!modalForm.occurrence) {
    modalErrors.occurrence = "Date and time are required.";
    return;
  }
  const payload = {
...
async function handleJoin(event: Event) {
  try {
    await store.joinEvent(event.id);
  } catch { }
}
async function handleLeave(event: Event) {
  try {
    await store.leaveEvent(event.id);
  } catch { }
}
</script>
